...
class Controller:
    ...
    def get_game_info(self, game_id):
        name = self.data_b.get("games", "name", "id", game_id)
        release_date = self.data_b.get("games", "release_date", "id", game_id)
        genre = self.data_b.get("games", "genre", "id", game_id)
        publisher = self.data_b.get("publishers", "publisher", "id", game_id)
        developer = self.data_b.get("publishers", "developer", "id", game_id)
        critic_rate = self.data_b.get("rates", "critic_rate", "id", game_id)
        user_rate = self.data_b.get("rates", "user_rate", "id", game_id)
        data = [name, release_date, genre, publisher, developer, critic_rate, user_rate]
        return data

    def get_game_platform_info(self, game_id):
        name = self.data_b.get("platforms", "name", "id", game_id)
    ...
    def get_user_rate_info(self, game_id):
        ...
        return round(average_user_score, 2)
